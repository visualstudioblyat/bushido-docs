...
Child webviews also have a fallback — `shortcut_bridge.js` captures keydown events and forwards them to Rust via `postMessage`, which then emits them as events to the main window.
